...
- (NSDate *) networkTime{
    ...
    NetAssociation *netAss = [[NetAssociation alloc] init:@"69.167.160.102"];
    NSDate *date = netAss.theServerDate;
    return    date;
   // [[sNSNotificationCenter defaultCenter] postNotificationName:@"net-time" object:self];
...
/*┌──────────────────────────────────────────────────────────────────────────────────────────────────┐
  ...
  └──────────────────────────────────────────────────────────────────────────────────────────────────┘*/
    ...
    {
        NSString *server = @"69.167.160.102";
        NetAssociation *    timeAssociation = [[[NetAssociation alloc] initWithServer:server withDelegate:self] autorelease];
//
        [timeAssociations addObject:timeAssociation];
        //[timeAssociation enable];   
        // starts are randomized internally
    }
...
-(void)updateDate
{
    NetAssociation *timeAssociation = [timeAssociations objectAtIndex:0];
    [timeAssociation enable];
}
#pragma mark- internalgetdatedelegate
-(void)internalGetDateDelegate:(NSDate *)date
{
    [getNTPDateDelegate getNTPDate:date];
}
